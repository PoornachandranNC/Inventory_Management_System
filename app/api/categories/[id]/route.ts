import { type NextRequest, NextResponse } from "next/server"
import { executeQuery, updateRow } from "@/lib/db"
import { isAdmin } from "@/lib/auth-server"

// GET a single category
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const id = params.id

    const query = "SELECT * FROM categories WHERE id = ?"
    const categories = await executeQuery<any[]>({ query, values: [id] })

    if (categories.length === 0) {
      return NextResponse.json({ error: "Category not found" }, { status: 404 })
    }

    return NextResponse.json(categories[0])
  } catch (error) {
    console.error("Error fetching category:", error)
    return NextResponse.json({ error: "Failed to fetch category" }, { status: 500 })
  }
}

// UPDATE a category
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    // Check if user is admin
    const admin = await isAdmin()

    if (!admin) {
      return NextResponse.json({ error: "Unauthorized. Admin access required." }, { status: 403 })
    }

    const id = params.id
    const body = await request.json()

    // Validate required fields
    if (!body.name) {
      return NextResponse.json({ error: "Category name is required" }, { status: 400 })
    }

    // Update the category
    const success = await updateRow({
      table: "categories",
      data: { name: body.name },
      whereClause: "id = ?",
      whereValues: [id],
    })

    if (!success) {
      return NextResponse.json({ error: "Category not found or no changes made" }, { status: 404 })
    }

    return NextResponse.json({
      success: true,
      id: Number(id),
      name: body.name,
    })
  } catch (error) {
    console.error("Error updating category:", error)
    return NextResponse.json({ error: "Failed to update category" }, { status: 500 })
  }
}

// DELETE a category
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    // Check if user is admin
    const admin = await isAdmin()

    if (!admin) {
      return NextResponse.json({ error: "Unauthorized. Admin access required." }, { status: 403 })
    }

    const id = params.id

    // Check if category exists
    const checkQuery = "SELECT id FROM categories WHERE id = ?"
    const categories = await executeQuery<any[]>({ query: checkQuery, values: [id] })

    if (categories.length === 0) {
      return NextResponse.json({ error: "Category not found" }, { status: 404 })
    }

    // Check if category is in use
    const usageQuery = "SELECT COUNT(*) as count FROM products WHERE category_id = ?"
    const usage = await executeQuery<any[]>({ query: usageQuery, values: [id] })

    if (usage[0].count > 0) {
      return NextResponse.json(
        {
          error: "Cannot delete category that is in use. Remove the category from all products first.",
        },
        { status: 400 },
      )
    }

    // Delete the category
    const deleteQuery = "DELETE FROM categories WHERE id = ?"
    await executeQuery({ query: deleteQuery, values: [id] })

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error("Error deleting category:", error)
    return NextResponse.json({ error: "Failed to delete category" }, { status: 500 })
  }
}
